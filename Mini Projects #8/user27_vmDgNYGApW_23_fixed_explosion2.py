...
def group_collide(myset, other_object):
    ...
    for each_item in list(myset):
        if each_item.collide(other_object):
                ...
                a_explode = Sprite(explode_pos, [0,0], 0, 0, explosion_image, explosion_info, explosion_sound)
                explosion_group.add(a_explode)
                
                if (type(other_object) == Ship):
                #print type(other_object)
                    a_explode = Sprite(other_object.get_position(), other_object.vel, 0, 0, explosion_image, explosion_info, explosion_sound)
                    #a_explode = Sprite(other_object.get_position(), [0,0], 0, 0, explosion_image, explosion_info, explosion_sound)
                    explosion_group.add(a_explode)
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            ...
            my_image_center[0] = self.image_center[0] + (self.image_size[0] * self.age)
    ...
    def update(self):
        ...
        #update age
        self.age += 1
        is_exceed_lifespan = self.age >= self.lifespan
        return is_exceed_lifespan
